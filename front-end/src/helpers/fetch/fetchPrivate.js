...
export const fetchPrivate = (endpoint, data, method = 'GET', header, contentType = 'application/json') => {
  return async (dispatch) => {
    const url = `${baseUrl}${endpoint}`;
    dispatch(loadingStart());
    const token = sessionStorage.token;
    const headers =
      method === 'GET' ? { 'x-token': token, ...header } : { 'content-type': contentType, 'x-token': token, ...header };
